...
namespace HaFT.Utilities
{
	...
	{
		private static bool ConvertToYN(string S) => string.Equals(S, "Yes", StringComparison.OrdinalIgnoreCase);
		...
		public static void Attr(this XmlNode Node, string Name, out bool? Value, bool? Default = null) => Node.Attr(Name, out Value, Default, S => ConvertToYN(S));
		public static void Attr(this XmlNode Node, string Name, out int? Value, int? Default = null) => Node.Attr(Name, out Value, Default, S => Convert.ToInt32(S));
		public static void Attr(this XmlNode Node, string Name, out char? Value, char? Default = null) => Node.Attr(Name, out Value, Default, S => Convert.ToChar(S));
		public static void Attr(this XmlNode Node, string Name, out float? Value, float? Default = null) => Node.Attr(Name, out Value, Default, S => Convert.ToSingle(S));
		#endregion
	}
}
